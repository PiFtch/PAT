...
struct Person {
    ...
    int Virtue_Grade;
    int Talent_Grade;
    Person(string idt, int virtue, int talent) {
        id = idt;
        Virtue_Grade = virtue;
        Talent_Grade = talent;
    }
...
bool cmp(Person a, Person b) {
    int suma = a.Virtue_Grade+a.Talent_Grade;
    int sumb = b.Virtue_Grade+b.Talent_Grade;
    if (suma != sumb)
        return suma > sumb;
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> id_temp >> virtue >> talent;
        if (virtue >= low && talent >= low) {
            if (virtue >= high && talent >= high) {
                Person p(id_temp, virtue, talent);
                // p.id = id_temp;
                // p.Virtue_Grade = virtue;
                // p.Talent_Grade = talent;
                sage.push_back(p);
            } else if (virtue >= high) {
                Person p(id_temp, virtue, talent);
                // p.id = id_temp;
    ...
    for (vector<Person>::iterator it = sage.begin(); it != sage.end(); it++) {
        cout << it->id << " " << it->Virtue_Grade << " " << it->Talent_Grade << endl;
    }
    for (vector<Person>::iterator it = nobleman.begin(); it != nobleman.end(); it++) {
        cout << it->id << " " << it->Virtue_Grade << " " << it->Talent_Grade << endl;
    }
    for (vector<Person>::iterator it = foolman.begin(); it != foolman.end(); it++) {
        cout << it->id << " " << it->Virtue_Grade << " " << it->Talent_Grade << endl;
    }
    for (vector<Person>::iterator it = smallman.begin(); it != smallman.end(); it++) {
        cout << it->id << " " << it->Virtue_Grade << " " << it->Talent_Grade << endl;
    }
}